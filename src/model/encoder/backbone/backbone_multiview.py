...
class BackboneMultiview(torch.nn.Module):
    ...
    def forward(
        ...
        # Feature-domain SemCom (insertion point B): transmit the per-view CNN
        # features through the channel before any cross-view fusion. Both the
        # cost-volume skip (cnn_features) and the transformer see the received
        # features.
        if feature_semcom is not None:
            stacked = torch.stack(cur_features_list, dim=1)  # [B, V, C, H, W]
            stacked = feature_semcom(stacked)
            cur_features_list = [stacked[:, i] for i in range(stacked.shape[1])]
