...
with install_import_hook(
    ...
    from src.model.semcom import get_semcom
...
def train(cfg_dict: DictConfig):
    ...
    if cfg.mode == "train" and checkpoint_path is not None and not cfg.checkpointing.resume:
        ...
        # With a SemCom module present, load non-strictly: the MVSplat checkpoint
        # has no semcom.* keys, so strict loading would fail; non-strict keeps the
        # pretrained JSCC weights already loaded into model_kwargs["semcom"].
        load_strict = cfg.semcom is None
        model_wrapper = ModelWrapper.load_from_checkpoint(
            checkpoint_path, **model_kwargs, strict=load_strict)
        print(cyan(f"Loaded weigths from {checkpoint_path} (strict={load_strict})."))
    else:
